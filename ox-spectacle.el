...
<head>
    ...
    window.addEventListener('load', function () {
        setTimeout(function() {
          ...
          style.type = 'text/css';
          if(style.styleSheet){
              style.styleSheet.cssText = tcss;
          } else {
              style.appendChild(document.createTextNode(tcss));
          }
          ...
          (concat " theme={theme} "
                  ...
                  (unless magicp
                    (let ((ps (split-string anim " ")))
                      (format " transition={[%s]} transionDuration='%s'"
...
CONTENTS holds the contents of the item.  INFO is a plist holding
contextual information."
  (concat "<ListItem>" (org-trim contents) "</ListItem>"))
